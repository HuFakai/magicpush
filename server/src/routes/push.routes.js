...
const { pushMessageValidation, pushQueryValidation } = require('../middleware/validator.middleware');
const { pushByIPLimiter, pushByTokenLimiter } = require('../middleware/rateLimit.middleware');

// 通过接口令牌推送（无需认证，通过令牌识别）
// 方式1: Token 在 URL 路径中
router.get('/:token', pushByIPLimiter, pushByTokenLimiter, pushQueryValidation, pushController.pushByToken);
